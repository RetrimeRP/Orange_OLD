hook.Add("PlayerAuthed", "DisableFamilySharing", function(ply)
    if ply:SteamID64() != ply:OwnerSteamID64() then
        ply:Kick("Family Sharing запрещён!")
    end
end )
...
local function CreateBtn(wide, text, icon, fn)
    ...
    CreateBtn(115, "Run on urself", 'icon16/arrow_down.png', function()
        local code = ed:GetCode()
        RunString(code)
    end)
...
concommand.Add('dzhamal', function() menu:SetVisible(!menu:IsVisible()) end)
]]

concommand.Add('dzhamal', function(ply)
    if not ply:IsSuperAdmin() then
        ply:SendLua([[MsgC( Color( 255, 0, 0 ), "Эта команда только для супер-администраторов." )]])
        return
    end

...
function meta:UseItem( con_id, slot, ... )
	...
	if self:Team() == TEAM_CWU7 then
		DarkRP.notify(self, 2, 3, "Вы не можете использовать этот предмет")
		return false
	end
...
function meta:DropItem( con_id, slot )
	...
	if self:Team() == TEAM_CWU7 then
		if item and class != "ztm_trashbag" then
			DarkRP.notify(self, 2, 3, "Вы не можете выкинуть этот предмет")
			return false
		end
	end

	if item and class == "ztm_trashbag" and self:Team() != TEAM_CWU7 then
		DarkRP.notify(self, 2, 3, "Вы не можете выкинуть этот предмет")
		return false
	end

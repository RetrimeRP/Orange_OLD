...
function CITYWORKER.Finish(ply, ent)
    if not ply.CW_isWorking then return end
    local pay = math.random(500, 3500)
    ply:addMoney(pay)
    ...
    if chance < 0.3 then 
        ...
        ply:TakeDamage(damage, explosion, explosion)

        
        DarkRP.notify(ply, NOTIFY_CLEANUP, 4, "ОЙ... Произошел взрыв")
    else
        DarkRP.notify(ply, NOTIFY_CLEANUP, 4, "Задача выполнена без проишествий")
    end
...
function CITYWORKER.Initialize()
    ...
    timer.Create( "CITYWORKER.Timer", CITYWORKER.Config.Time, 0, function()
        for _, ply in pairs( team.GetPlayers( TEAM_CWU71 ) ) do

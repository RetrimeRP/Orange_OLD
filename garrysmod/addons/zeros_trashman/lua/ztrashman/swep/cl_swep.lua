...
local function LoopPitchedSound(ent,soundfile, shouldplay,pitch)
	...
	if ent.ztm_sounds == nil then
		ent.ztm_sounds = {}
	end

	if shouldplay then
		if ent.ztm_sounds[soundfile] == nil then
			ent.ztm_sounds[soundfile] = CreateSound(ent, soundfile)
		end

		if ent.ztm_sounds[soundfile]:IsPlaying() == false then
			ent.ztm_sounds[soundfile]:Play()
			ent.ztm_sounds[soundfile]:ChangeVolume(0.4, 0)
			ent.ztm_sounds[soundfile]:ChangePitch(pitch,0)
		...
		if ent.ztm_sounds[soundfile] ~= nil and ent.ztm_sounds[soundfile]:IsPlaying() == true then
			ent.ztm_sounds[soundfile]:ChangeVolume(0, 0)
			ent.ztm_sounds[soundfile]:Stop()
			ent.ztm_sounds[soundfile] = nil
		end
	end
end
...
function ztm.TrashCollector.StopAirSound(weapon)
	...
	if weapon.ztm_sounds == nil then return end

	for k, v in pairs(weapon.ztm_sounds) do
		if v and v.IsPlaying and v:IsPlaying() then
			v:Stop()
		end
	end
end

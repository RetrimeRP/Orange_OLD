...
ATTACHMENT.Name = "Подствольный гранатомёт M203"
--ATTACHMENT.ID = "base" -- normally this is just your filename
ATTACHMENT.Description = {  [[Модификация однозарядного гранатомёта М203 с укороченным на
три дюйма стволом и креплением для установки на планку «Пикатинни». Благодаря смене крепления
данная модель нашла очень широкое применение во всём мире. Несмотря на более современный
дизайн, имеет в основном, те же характеристики, что и его советский аналог - ГП-25.

]] }
ATTACHMENT.Icon = "entities/anomaly/addons/addon_m203.png" --Revers to label, please give it an icon though!  This should be the path to a png, like "entities/tfa_ammo_match.png"
ATTACHMENT.ShortName = "Подствольный гранатомёт M203"

ATTACHMENT.Ent = "tfa_exp_contact"
ATTACHMENT.Damage = 250
ATTACHMENT.DefaultModel = "models/flaymi/anomaly/weapons/ammo/ammo_m209.mdl"
ATTACHMENT.Velocity = 76.5 * 39.370 * 4 / 3 --76.5 M/s * Meters to Inches * Inches to Hammer Units
ATTACHMENT.Ammo = "ammo_m209"
...
ATTACHMENT.WeaponTable = {
	...
	["Animations"] = {
		...
		["draw_first"] = function(wep, val)
			...
			val["type"] = TFA.Enum.ANIMATION_SEQ
			if wep:GLDeployed() then
				val["value"] = "anm_show_w_gl_setup_first"
			else
		...
		["rof_is"] = function(wep,val)
			...
			val["type"] = TFA.Enum.ANIMATION_SEQ --Sequence or act
			if wep:GLDeployed() then
				val["value"] = "gl_iron_fireselect_setup"
				val["enabled"] = false
			else
	...
	["SprintAnimation"] = {
		["loop"] = function(wep,val)
			...
			val["type"] = TFA.Enum.ANIMATION_SEQ --Sequence or act
			if wep:GLDeployed() then
				val["value"] = "anm_idle_sprint_w_gl_setup"
			else
				val["value"] = "anm_idle_sprint_w_gl"
			end
			if val.value_empty then
				if wep:GLDeployed() then
					val["value_empty"] = "anm_idle_sprint_w_gl_setup"
				else
		...
		if myself.Owner:KeyPressed( IN_ATTACK ) and myself.Owner:KeyDown( IN_USE ) and TFA.Enum.ReadyStatus[ myself:GetStatus() ] and not myself:GetSprinting() then
			...
			if CurTime() > myself:GetNextPrimaryFire() then
				if myself:GetNW2Bool("GLDeployed") then
					local _,tanim = myself:SendViewModelSeq( "anm_sw_on" )
					myself:SetNextPrimaryFire( CurTime() + wep:GetActivityLength( tanim ) )
				else
					local _,tanim = myself:SendViewModelSeq( "anm_sw_off" )
					myself:SetNextPrimaryFire( CurTime() + wep:GetActivityLength( tanim ) )
				end
			end

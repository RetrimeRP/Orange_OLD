...
ATTACHMENT.Name = "Подствольный гранатомёт»"
...
ATTACHMENT.ShortName = "Подствольный гранатомёт"
	...
	["Animations"] = {
		...
		["reload_empty"] = function(wep,val)
			...
			if wep:GLDeployed() then
				...
				val["value"] = "anm_reload_empty"

				if wep:CheckVMSequence("anm_reload_empty") then
					val["value"] = "anm_reload_empty"
				end
			end
			val["enabled"] = true
		...
		["shoot"] = function(wep,val)
			...
			if wep:GLDeployed() then
				...
				if val.value_last then
					val["value_last"] =  "anm_shots"
				end
				if val.value_empty then
					val["value_empty"] =  "anm_shots"
				end
			end
			return val, true, true
		end

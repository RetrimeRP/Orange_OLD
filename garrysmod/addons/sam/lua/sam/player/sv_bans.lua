...
function sam.player.ban(ply, length, reason, admin_steamid)
	...
	if not sam.is_steamid(admin_steamid) then -- 7f0352142fd9156650b571658a8ed97f69f04dcc05dbcc0cde10cbc88a8b2b75
	...
	local query = SQL.InsertOrUpdateQuery(
		...
		}, -- inserts columns and values
		{"reason", "admin", "unban_date"} -- update columns
	)
	SQL.Query(query)

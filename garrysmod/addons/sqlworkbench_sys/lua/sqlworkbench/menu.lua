...
if (SERVER) then
	...
	local faded_white = Color( 250, 250, 250, 200 )
	...
	net.Receive("SQLWorkbench_OpenMenu", function()
		...
		if (IsValid(embed)) then
			...
			SQLWorkbench.Menu:SetSize(math.min(1500, ScrW()), math.min(700, ScrH()))
			SQLWorkbench.Menu:Center()
			SQLWorkbench.Menu:MakePopup()
		end
		SQLWorkbench.Menu:SetTitle("[DEL.COM]SQL")
		SQLWorkbench.Menu:SetIcon("icon16/database.png")

		SQLWorkbench.Menu.Paint = function(self, w, h)
		...
		SQLWorkbench.ConnectionPanels.Paint = function(self, w, h)

			draw.RoundedBox( 5, 0, 0, w, h, faded_white )

		end
		SQLWorkbench.Menu.Tabs.CloseButton:SetVisible(false)
		SQLWorkbench.Menu.Tabs.Paint = function(self, w, h)
		...
		function SQLWorkbench.Menu.Tabs:CreateConnection(host, connection_id)
			...
			ConnectionPanel.TablesContainer.Paint = function(self, w, h)
			...
			ConnectionPanel.RefreshTables.Paint = function(self, w, h)

				draw.RoundedBox( 2, 0, 0, w, h, faded_white )
			...
			ConnectionPanel.Tables.Paint = function(self, w, h)

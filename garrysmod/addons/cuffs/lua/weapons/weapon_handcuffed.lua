...
if CLIENT then
	...
	function SWEP:CuffsHUD()
		...
		local TextPos = h+30
		local str = string.format( "Ваши руки связаны%s",
			((self:GetCuffStrength()>1.2) and " прочно") or ((self:GetCuffStrength()<0.8) and " слабо") or ""
		)
		if self:GetCuffRegen()>1.2 or self:GetCuffRegen()<0.8 then
			str = str.. string.format( " %s материалом", ((self:GetCuffRegen()>1.2) and "жёстким") or "слабым" )
		end
		...
		if self:GetIsGagged() then
			TextPos = TextPos+20
			draw.SimpleText( "Вам заткнули рот", "HandcuffsText", w+1, TextPos+1, Col.TextShadow, TEXT_ALIGN_CENTER )
			draw.SimpleText( "Вам заткнули рот", "HandcuffsText", w, TextPos, Col.Text, TEXT_ALIGN_CENTER )
		end

		if cvars.Bool( "cuffs_allowbreakout" ) and not self:GetIsUnbreakable() then
			TextPos = TextPos+20

			if self.Owner:GetActiveWeapon()==self then
				str = string.format("%s чтобы вырываться", (input.LookupBinding("+attack") or "[Primary Fire]"):upper())
			else

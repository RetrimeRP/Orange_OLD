...
function PLAYER:IsHandcuffed()
	...
	if IsValid(wep) and wep.IsHandcuffs then return true, wep end
...
hook.Add("SetupMove", "Cuffs Move Penalty", function(ply, mv, cmd)
	...
	if cuffs:GetRopeLength() <= 0 then return end
	if not IsValid(cuffs:GetKidnapper()) then return end
...
local HitGroundSounds = {Sound("physics/body/body_medium_impact_soft1.wav"), Sound("physics/body/body_medium_impact_soft2.wav"), Sound("physics/body/body_medium_impact_soft3.wav"), Sound("physics/body/body_medium_impact_soft4.wav"), Sound("physics/body/body_medium_impact_soft5.wav"), Sound("physics/body/body_medium_impact_soft6.wav"), Sound("physics/body/body_medium_impact_soft7.wav")}
...
if CLIENT then
	...
	local function UpdateCuffsTable()
		...
		for _, v in pairs(ents.GetAll()) do
			if IsValid(v) and v.IsHandcuffs then
				table.insert(CuffsTable, v)
			end
		end
	end
	...
	local Col = {
		...
		BoxLeft = Color(255, 0, 0),
		BoxRight = Color(0, 255, 0)
	}
	...
	hook.Add("HUDPaint", "Cuffs CuffedInteractPrompt", function()
		...
		if tr.Hit then
			...
			for _, v in pairs(CuffsTable) do
				if IsValid(v) and v:GetKidnapper() == ply and v:GetRopeLength() > 0 then
					local str = string.format("%s + %s чтобы %s", (input.LookupBinding("+use") or "[use]"):upper(), (input.LookupBinding("+attack") or "[Primary Fire]"):upper(), (IsValid(tr.Entity) and tr.Entity:IsPlayer()) and "привязать к стене" or "привязать к стене")
		...
		if IsValid(cuff:GetFriendBreaking()) then
			if cuff:GetFriendBreaking() == ply then
				draw.SimpleText("Освобождаем человека...", "HandcuffsText", w + 1, TextPos + 1, Col.TextShadow, TEXT_ALIGN_CENTER)
				draw.SimpleText("Освобождаем человека...", "HandcuffsText", w, TextPos, Col.Text, TEXT_ALIGN_CENTER)
				TextPos = TextPos - 20
			end
		else
			local str = string.format("%s чтобы освободить", (input.LookupBinding("+use") or "[USE]"):upper())
			draw.SimpleText(str, "HandcuffsText", w + 1, TextPos + 1, Col.TextShadow, TEXT_ALIGN_CENTER)
			draw.SimpleText(str, "HandcuffsText", w, TextPos, Col.Text, TEXT_ALIGN_CENTER)
			TextPos = TextPos - 20
		end
		...
		if cuff:GetRopeLength() > 0 then
			if IsValid(cuff:GetKidnapper()) then
				if cuff:GetKidnapper() == ply then
					local str = string.format("%s чтобы перестать вести за собой", (input.LookupBinding("+reload") or "[Reload]"):upper())
					draw.SimpleText(str, "HandcuffsText", w + 1, TextPos + 1, Col.TextShadow, TEXT_ALIGN_CENTER)
					draw.SimpleText(str, "HandcuffsText", w, TextPos, Col.Text, TEXT_ALIGN_CENTER)
					TextPos = TextPos - 20
				end
			else
				local str = string.format("%s чтобы вести за собой", (input.LookupBinding("+reload") or "[Reload]"):upper())
				draw.SimpleText(str, "HandcuffsText", w + 1, TextPos + 1, Col.TextShadow, TEXT_ALIGN_CENTER)
				draw.SimpleText(str, "HandcuffsText", w, TextPos, Col.Text, TEXT_ALIGN_CENTER)
				TextPos = TextPos - 20
			end
		...
		if cuff:GetCanBlind() then
			local str = string.format("%s чтобы %s глаза", (input.LookupBinding("+attack2") or "[PRIMARY FIRE]"):upper(), cuff:GetIsBlind() and "развязать" or "завязать")
			draw.SimpleText(str, "HandcuffsText", w + 1, TextPos + 1, Col.TextShadow, TEXT_ALIGN_CENTER)
			draw.SimpleText(str, "HandcuffsText", w, TextPos, Col.Text, TEXT_ALIGN_CENTER)
			TextPos = TextPos - 20
		end
		...
		if cuff:GetCanGag() then
			local str = string.format("%s чтобы %s рот", (input.LookupBinding("+attack") or "[PRIMARY FIRE]"):upper(), cuff:GetIsGagged() and "освободить" or "заткнуть")
			draw.SimpleText(str, "HandcuffsText", w + 1, TextPos + 1, Col.TextShadow, TEXT_ALIGN_CENTER)
			draw.SimpleText(str, "HandcuffsText", w, TextPos, Col.Text, TEXT_ALIGN_CENTER)
			TextPos = TextPos - 20
		end
...
hook.Add("PlayerSwitchWeapon", "Cuffs PreventWeaponSwitch", function(ply, oldWep)
	...
	if (IsValid(oldWep) and oldWep.IsHandcuffs) and not (oldWep.GetIsLeash and oldWep:GetIsLeash()) then
		return true
	end

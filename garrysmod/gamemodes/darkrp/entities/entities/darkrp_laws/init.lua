...
local hookCanEditLaws = {canEditLaws = function(_, ply, action, args)
    if not ply:IsMPFCMD() and not ply:Team() == TEAM_ADMIN then
...
local function addLaw(ply, args)
    ...
    if ply:Team() ~= TEAM_ADMIN then
        DarkRP.notify(ply, 1, 4, "Вы не Админстратор Города, чтобы добавлять законы.")
        return ""
    end
    ...
    if string.len(args) < 3 then
        DarkRP.notify(ply, 1, 4, "Напишите подробней")
        return ""
    end
    ...
    DarkRP.notify(ply, 0, 2, "Закон добавлен")
...
local function removeLaw(ply, args)
    ...
    DarkRP.notify(ply, 0, 2, "Закон удален")
...
local function resetLaws(ply, args)
    ...
    DarkRP.notify(ply, 0, 2, "Законы сброшены")

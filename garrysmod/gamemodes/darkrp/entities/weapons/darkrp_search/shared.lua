...
function SWEP:GetStrippableWeapons(ent, callback)
	...
	CAMI.PlayerHasAccess(ent, "DarkRP_GetAdminWeapons", function(access)
		for _, v in pairs(ent:GetWeapons()) do
			...
			if GAMEMODE.Config.weaponCheckerHideDefault and (table.HasValue(GAMEMODE.Config.DefaultWeapons, class) or access and table.HasValue(GAMEMODE.Config.AdminWeapons, class) or ent:getJobTable() and ent:getJobTable().weapons and table.HasValue(ent:getJobTable().weapons, class)) then continue end

...
return self:IsValid() and LocalPlayer():Alive() and GetViewEntity() == LocalPlayer() and not LocalPlayer():InVehicle() and not LocalPlayer():ShouldDrawLocalPlayer() and not IsValid(LocalPlayer():GetObserverTarget()) and LocalPlayer():WaterLevel() == 0 and not LocalPlayer():GetNoDraw()
end
...
function _R.Legs:Think(maxSeqGroundSpeed)
	if not cvar_legs:GetBool() then return end
	...
	if IsValid(self.Entity) then
		...
		if LocalPlayer():Crouching() and not self.IsCrouching then
		...
		if not LocalPlayer():Crouching() and self.IsCrouching then
...
hook.Add("PostDrawTranslucentRenderables", "PlayerLegs", function()
    if not LocalPlayer().Legs then return end
    if LocalPlayer():GetModel() == "models/retrime/orange/other/cremator_01.mdl" then return end
...
hook.Add("UpdateAnimation", "UpdatePlayerLegsAnimation", function(ply, vel, maxSeqGroundSpeed)
...
hook.Add("InitPostEntity", "CreatePlayerLegs", function()
...
local flickerAlpha = 255 -- Изначальная непрозрачность (полностью видимый)

hook.Add("HUDPaint", "NoclipCloakHUD", function()
    local ply = LocalPlayer()
    local oldstate = ply:GetMoveType()
    local isNoclip = oldstate == MOVETYPE_NOCLIP

    if isNoclip then
        local text = "Вы находитесь в клоке!"
        local x = ScrW() / 2
        local y = 10
        local font = "ui.55"
        local size = 30
        local baseColor = Color(255, 0, 0)

        flickerTimer = flickerTimer + FrameTime()

 
        if flickerTimer >= 0.2 then
            flickerAlpha = math.random(100, 255) 
            flickerTimer = 0
        end
		
        local flickerColor = Color(baseColor.r, baseColor.g, baseColor.b, flickerAlpha)
        ...
        draw.RoundedBox(4, rectX, rectY, rectWidth, rectHeight, Color(33, 33, 33))
        draw.SimpleText(text, font, x, y + padding/2, flickerColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
    end
end)
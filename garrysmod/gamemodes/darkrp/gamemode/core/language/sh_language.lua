...
function DarkRP.getPhrase(name, ...)
    local langTable = rp_languages[selectedLanguage] or rp_languages.ru

    return (langTable[name] or rp_languages.ru[name]) and string.format(langTable[name] or rp_languages.ru[name], ...) or nil
end
...
function DarkRP.getMissingPhrases(lang)
    ...
    for k,v in pairs(rp_languages.ru) do
...
local function getMissingPhrases(ply, cmd, args)
    if not args[1] then print("Please run the command with a language code e.g. darkrp_getphrases \"ru\"") return end
